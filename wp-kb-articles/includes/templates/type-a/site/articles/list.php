			...
			<div class="-tabs">
				<ul class="-list">
					<?php foreach($tab_categories as $_tab_category): ?>
						<li>
							<a href="#" data-category="<?php echo esc_attr($_tab_category->term_id); ?>"
								<?php if(in_array((integer)$_tab_category->term_id, $attr->category, TRUE)): ?> class="-active"<?php endif; ?>
								><?php echo esc_html($_tab_category->name); ?></a>
						</li>
					<?php endforeach; // End category iteration.
					unset($_tab_category); // Housekeeping. ?>
				</ul>
			</div>
		<?php endif; ?>
			...
			<div class="-tags">
				...
				<div class="-overlay">
					<div class="-selected">
						...
						<?php $_selected_tags = ''; // Initialize.

						foreach($attr->tag as $_tag_id) // Iterate tags in query.
							foreach($tags as $_tag) if($_tag_id === (integer)$_tag->term_id)
							{
								$_selected_tags .= ($_selected_tags ? ', ' : '').esc_html($_tag->name);
								break; // Break the inner iteration; we found this tag.
							}
						if(!$_selected_tags) // There are no tags selected right now?
							$_selected_tags = '<strong>'.__('None', $plugin->text_domain).'</strong>'.
							                  ' '.__('(select some tags) and click `filter by tags`', $this->plugin->text_domain);
						echo $_selected_tags; // Currently selected tags.

						unset($_tag, $_selected_tags); // Housekeeping. ?>
					</div>
					<ul class="-list">
						<?php foreach($tags as $_tag): ?>
							<li><a href="#" data-tag="<?php echo esc_attr($_tag->term_id); ?>"
									<?php if(in_array((integer)$_tag->term_id, $attr->tag, TRUE)): ?> class="-active"<?php endif; ?>
									><?php echo esc_html($_tag->name); ?></a></li>
						<?php endforeach; // End tag iteration.
						unset($_tag); // Housekeeping. ?>
					</ul>
			...
			<?php while($query->have_posts()): $query->the_post(); ?>
				<div class="-article">
					...
					<div class="-meta">
						...
						<?php if(($_tags = get_the_terms(get_the_ID(), $plugin->post_type.'_tag'))): ?>
							<div class="-tags">
								<span><?php echo __('tagged:', $plugin->text_domain); ?></span>
								<?php $_tagged = ''; // Initialize.
								foreach($_tags as $_tag) // Iterate the tags that it has.
									$_tagged .= ($_tagged ? ', ' : ''). // Comma-delimited tags.
									            '<a href="#" data-click-tag="'.esc_attr($_tag->term_id).'">'.esc_attr($_tag->name).'</a>';
								echo $_tagged; // Display the tags now; with possible commas.
								unset($_tagged, $_tag); // Housekeeping. ?>
							</div>
